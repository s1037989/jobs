...
#
#   TEST_ONLINE=postgres://tester:testing@/test script/jobs test
#
...
# Isolate tests
my $url
  = Mojo::URL->new($ENV{TEST_ONLINE})->query([search_path => 'mojo_jobs_test']);
my $pg = Mojo::Pg->new($url);
$pg->db->query('drop schema if exists mojo_jobs_test cascade');
$pg->db->query('create schema mojo_jobs_test');

# Override configuration for testing
my $t = Test::Mojo->new(Jobs => {pg => $url, secrets => ['test_s3cret']});
$t->ua->max_redirects(10);

# No posts yet
$t->get_ok('/')->status_is(200)->text_is('title' => 'Jobs')
...
$t->post_ok('/posts/1?_method=DELETE')->status_is(200)
  ->text_is('title' => 'Jobs')->element_exists_not('h2');

# Clean up once we are done
$pg->db->query('drop schema mojo_jobs_test cascade');
